...
#[tokio::test]
async fn thread_channel_append_emits_notification_and_can_inject_model_context() -> Result<()> {
    ...
    let append_req = mcp
        .send_thread_channel_append_request(ThreadChannelAppendParams {
            ...
            message: ThreadChannelMessageInput {
                ...
                model_text: Some("Model-visible peer payload".to_string()),
    ...
    assert_eq!(item.id, "channel-1");
    assert_eq!(item.channel, "peer-inbox");
    assert_eq!(item.sender, "codex-peer");
    assert_eq!(item.sender_kind, ChannelSenderKind::Agent);
    assert_eq!(item.text, "hello from peer");
    assert_eq!(item.preview.as_deref(), Some("hello"));
    ...
    let model_input = serde_json::to_string(&response_mock.single_request().input())?;
    assert!(

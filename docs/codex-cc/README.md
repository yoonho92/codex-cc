...
version. `CODEX_CLI_VERSION_OVERRIDE` is the upstream Codex version the binary
is based on. `codex-cc --version` shows both values when the package build sets
`CODEX_CC_DISPLAY_VERSION`.
...
`codex-cc-version.json` cache so upstream Codex release versions cannot be
compared against the Codex-CC package version.

Automation launchers that need non-blocking TUI startup can set
...
- MCP/app notification parsing for channel payloads using fields such as
  ...
  optional `modelText`.
- MCP tool-result presentation metadata for compact human-facing rendering
  while retaining raw result data for audit and replay.
